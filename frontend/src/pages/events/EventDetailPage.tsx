    ...
    return(
        ...
        <DashboardLayout>
            <div className="flex justify-center items-start px-4 w-full">
                <button onClick={onClose} className='text-[1.3rem] font-semibold mr-6'>←</button>
                <h2 className='text-[1.4rem] font-bold mx-10'>
                    {eventDetails.event_name}
                </h2>
                {canManage && (
                    <div className="flex gap-3">
                        <button className="text-[1rem] font-semibold px-4 py-2 rounded-lg border" onClick={() => setShowAlbumModal(true)}>+ Create Album</button>
                        <button className="text-[1rem] font-semibold px-4 py-2 rounded-lg border" onClick={() => setShowTag(true)}>+ Tag People</button>
                    </div>
                )}
            </div>
            { canManage && !selectionMode && (
                <div className="my-4 px-10 w-full">
            ...
            {!selectionMode ? (
                ...
                <div className="flex justify-center items-center gap-6 w-full">
                    ...
                    }} className="px-6 py-2 bg-gray-300">Select Albums</button>

                    <button onClick={() => {
                        clear();
                        setSelectionMode(true);
                        setActiveSelection("photo");
                    }} className="px-6 py-2 bg-gray-300">Select Photos</button>
                </div>
                </>
            ) : (
                <button onClick={() => {
            ...
            <div className="flex flex-col items-start justify-center gap-6 w-full">
                <div className="text-[1.2rem] font-semibold mb-2">Albums</div>
            ...
            <div className="flex flex-col items-start justify-center my-6 w-full">
                ...
                <div className="grid lg:grid-cols-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    {photos.map((photo) => (
                        <SelectableCard key={photo.photo_id} id = {photo.photo_id} disabled={activeSelection !== "photo"} onClick={ () => {
                            ...
                            <img src={photo.file_original} alt="photo" className="w-full aspect-square object-cover rounded" />
                        </SelectableCard>
                    ))}
                </div>
            </div>
